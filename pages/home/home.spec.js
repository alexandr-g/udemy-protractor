...
describe('Home page : login/register', function() {
    ...
    it('should try sign in and verify that password error appear', function() {
        homePage.goToLogin();
    ...
    it('should try sign in and vefiry that password cant be blank error appear', function() {
        homePage.goToLogin()
        helper.waitUntilReady(homePage.usernameField);
        homePage.usernameField.sendKeys('123@test.com');
        helper.waitUntilReady(homePage.signInButton)
        homePage.signInButton.click()
        helper.waitUntilReady(homePage.passwordExistingError)
        expect(homePage.passwordExistingError.getText()).toBe(homePage.passwordBlankText)
    });

    it('should try sign in and vefiry that email cant be blank error appear', function() {
        homePage.goToLogin()
        homePage.userPasswordField.sendKeys('something')
        helper.waitUntilReady(homePage.signInButton)
        homePage.signInButton.click()
        helper.waitElementToBeVisisble(homePage.passwordExistingError)
        expect(homePage.passwordExistingError.getText()).toBe(homePage.passwordBlankText)
    });
